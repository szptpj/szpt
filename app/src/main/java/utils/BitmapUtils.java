...
//      return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
    ...
    public static  Bitmap compressImage(Bitmap image) {
        ...
        int options = 100;
        while ( baos.toByteArray().length / 1024>100&&options>2) {    //循环判断如果压缩后图片是否大于100kb,大于继续压缩
            baos.reset();//重置baos即清空baos
            options /= 2;//每次都减少10
    ...
    //动画制图
    public static Bitmap compBig(Bitmap image) {
        ...
        isBm = new ByteArrayInputStream(baos.toByteArray());
        bitmap = BitmapFactory.decodeStream(isBm, null, newOpts);
        return compressImageBig(bitmap);//压缩好比例大小后再进行质量压缩
    }

    public static Bitmap compressImageBig(Bitmap image) {
